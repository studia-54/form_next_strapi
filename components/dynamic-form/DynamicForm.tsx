...
export const DynamicForm: React.FC<DynamicFormProps> = ({ fields }) => {
    ...
    const [selectedRadioItemId, setSelectedRadioItemId] = useState<
        number | null
    >(null);
    ...
    const renderField = (question: Question) => {
        switch (question.type) {
                            ...
                            {question.options.map((option) => (
                                <CheckboxItem
                                    ...
                                    onClick={() => {
                                        ...
                                        if (
                                            newSelectedCheckboxes?.includes(
                                                option.id
                                            )
                                        ) {
                                            const index =
                                                newSelectedCheckboxes.indexOf(
                                                    option.id
                                                );

                                            if (index > -1) {
                                                newSelectedCheckboxes.splice(
                                                    index,
                                                    1
                                                );
                                            }
            ...
            case 'radiogroup':
                return (
                    <div
                        className={styles.form__radiobuttons}
                        key={question.id.toString()}
                    >
                        {question.options.map((option) => (
                            <RadiogroupItem
                                ...
                                onClick={() =>
                                    setSelectedRadioItemId(option.id)
                                }
                            />
                        ))}
                    </div>
                        ...
                        {fields.questions.map((field, index) => {
                            ...
                            return (
                                <Fragment key={field.id}>
                                    ...
                                    <div
                                        className={
                                            styles.form__errors_container
                                        }
                                    >
                                        {errors[`${type}:${field.id}`] && (
                                            <p
                                                className={
                                                    styles.form__error_text
                                                }
                                            >
                                                {
                                                    errors[
